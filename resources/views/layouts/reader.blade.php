    ...
    <div class="flex h-screen overflow-hidden text-gray-900">
        ...
        <aside class="w-64 bg-white border-r border-black hidden md:block">
            ...
            <nav class="mt-6 px-4 space-y-2">
                ...
                <!-- Pesan & Peringatan -->
                @php $unreadWarnings = \App\Models\Warning::where('user_id', auth()->id())->where('is_read', false)->count(); @endphp
                <a href="{{ route('warnings.index') }}" class="flex items-center px-4 py-2.5 text-sm font-bold transition-colors {{ request()->routeIs('warnings.*') ? 'bg-white border-l-4 border-[#FF3B30] text-[#FF3B30]' : 'text-black hover:bg-gray-100 border-l-4 border-transparent' }}">
                    ...
                    Pesan
                    @if($unreadWarnings > 0)
                        <span class="ml-auto bg-[#FF3B30] text-white text-[10px] font-bold w-5 h-5 flex items-center justify-center rounded-full">{{ $unreadWarnings }}</span>
                    @endif
                </a>
            </nav>
        </aside>
        ...
        <div class="flex-1 flex flex-col h-screen overflow-hidden">
            ...
            <header class="hidden md:flex h-16 items-center justify-between px-6 bg-white border-b border-black">
                ...
                <div class="flex items-center gap-4">
                    ...
                    @php $unreadNavWarnings = \App\Models\Warning::where('user_id', auth()->id())->where('is_read', false)->count(); @endphp
                    <a href="{{ route('warnings.index') }}" class="relative hover:opacity-80 transition" title="Pesan & Peringatan">
                        ...
                        @if($unreadNavWarnings > 0)
                            <span class="absolute -top-1 -right-1 bg-[#FF3B30] text-white text-[10px] font-bold w-4 h-4 flex items-center justify-center rounded-full animate-pulse">{{ $unreadNavWarnings }}</span>
                        @endif
                    </a>
                ...
                </div> <!-- close flex items-center gap-4 -->
            </header>
